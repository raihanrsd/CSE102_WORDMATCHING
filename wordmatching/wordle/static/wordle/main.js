...
fetch('/static/wordle/words.txt')
    ...
    .then(text => {
        ...
        for (const word in words) {
            if (words[word].trim().length === info.word_amount && validateWord(words[word].trim())) {

                dict.set(words[word].trim().toLowerCase(), true);
            }
        }
    });
...
async function loadWords(letter_num, file_path) {
    ...
    for (const word in words) {
        if (words[word].trim().length === letter_num) {
            count += 1;
            words_arr.push(words[word].trim());
        }
    }
